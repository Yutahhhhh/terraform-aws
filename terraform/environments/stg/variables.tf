...
variable "enable_db_multi_az" {
  ...
  type        = bool
  default     = false  # ステージング環境ではコスト削減のためfalse
}
...
variable "enable_scheduled_scaling" {
  ...
  type        = bool
  default     = false  # ステージング環境ではデフォルトで無効
}
...
variable "skip_final_snapshot" {
  ...
  type        = bool
  default     = false  # ステージング環境ではスナップショットを作成
}
